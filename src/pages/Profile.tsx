...
import { Camera, Save, Mail, Calendar, MessageSquare, BookOpen, StickyNote, Image as ImageIcon } from "lucide-react";
    ...
    <PageShell title="Профайл">
      <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <Card className="p-6 bg-card/60 backdrop-blur-xl border-border/50">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="relative">
              <Avatar className="w-24 h-24 ring-2 ring-primary/40 shadow-[0_0_30px_hsl(var(--primary)/0.4)]">
              ...
              <button
                ...
                disabled={uploading}
                className="absolute -bottom-1 -right-1 w-9 h-9 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg hover:scale-105 transition-transform disabled:opacity-50"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <StatCard icon={MessageSquare} label="Яриа" value={stats.chats} />
          <StatCard icon={BookOpen} label="Үзсэн хичээл" value={stats.lessons} />
          <StatCard icon={StickyNote} label="Тэмдэглэл" value={stats.notes} />
          <StatCard icon={ImageIcon} label="AI зураг" value={stats.images} />
        </div>
      </div>
    </PageShell>
  );
...
const StatCard = ({ icon: Icon, label, value }: { icon: any; label: string; value: number }) => (
  <Card className="p-4 bg-card/60 backdrop-blur-xl border-border/50 hover:border-primary/40 transition-colors">
